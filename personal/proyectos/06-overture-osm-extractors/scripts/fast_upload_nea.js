...
function formatArgentineMobile(raw, defaultPref = '3794', index = 0) {
  let clean = (raw || '').replace(/\D/g, '');
  if (!clean || clean.length < 6) {
    const suffix = String(100000 + ((index * 4127) % 899999)).slice(0, 6);
...
async function main() {
  ...
  console.log(`Total comercios seleccionados para el NEA Extendido: ${selectedPool.length}`);
  ...
  for (let i = 0; i < finalRows.length; i += batchSize) {
    ...
    console.log('Respuesta lote:', JSON.stringify(resp).slice(0, 100));
  }
