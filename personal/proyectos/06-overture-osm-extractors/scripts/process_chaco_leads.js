...
const NUMVALIDATE_URL = 'http://157.151.13.179:8088/validate';
const WEBHOOK_APPEND = 'https://n8n.santiagowuerich.info/webhook/prospeccion-append-leads';

// Coordenadas de ciudades de Chaco para geocodificación reversa rápida si falta addr:city
const CHACO_CITIES = [
...
function getDolorPorRubro(rubro) {
  ...
  if (r.includes('indumentaria') || r.includes('showroom') || r.includes('ropa') || r.includes('moda') || r.includes('calzado') || r.includes('clothes')) {
    return 'Pérdida de tiempo enviando fotos y consultando talles/stock sin catálogo online.';
  }
  if (r.includes('estética') || r.includes('peluquer') || r.includes('barber') || r.includes('belleza') || r.includes('beauty') || r.includes('hairdresser')) {
    return 'Ausentismo a turnos y saturación de agenda por coordinar citas manualmente.';
  }
  if (r.includes('muebl') || r.includes('deco') || r.includes('hogar') || r.includes('furniture')) {
    return 'Cotizaciones lentas por WhatsApp y falta de catálogo con medidas/materiales.';
  }
  if (r.includes('gastronom') || r.includes('restaurant') || r.includes('comida') || r.includes('bar') || r.includes('fast_food')) {
    return 'Comisiones altas de apps de delivery y cuellos de botella tomando pedidos a mano.';
  }
  if (r.includes('taller') || r.includes('automot') || r.includes('car_repair') || r.includes('repuesto')) {
    return 'Falta de seguimiento automático de presupuestos y demoras informando reparaciones.';
  }
  if (r.includes('distribuidora') || r.includes('mayorista') || r.includes('supermercado') || r.includes('kiosco')) {
    return 'Errores al tomar pedidos por texto/audio y listas de precios desactualizadas.';
  }
  if (r.includes('farmacia') || r.includes('veterinaria') || r.includes('salud') || r.includes('dentist')) {
    return 'Demoras en confirmación de turnos y consultas repetitivas sobre coberturas/horarios.';
  }
...
function validateWithNumValidate(phone, country = 'AR') {
  return new Promise((resolve) => {
    if (!phone) return resolve({ valid: false });
    const url = `${NUMVALIDATE_URL}?number=${encodeURIComponent(phone)}&country=${country}`;
    http.get(url, (res) => {
      let body = '';
...
async function run() {
  const rawPath = '/Users/santi/.gemini/antigravity-ide/brain/e23bfcb1-03cf-46e1-adee-73457ec8e50f/scratch/chaco_osm_raw.json';
  if (!fs.existsSync(rawPath)) {
    console.log('Esperando archivo raw...');
    return;
  }
  
  const rawElements = JSON.parse(fs.readFileSync(rawPath, 'utf8'));
  console.log(`Procesando ${rawElements.length} elementos de OpenStreetMap...`);
  ...
  for (const el of rawElements) {
    ...
    // Filtros de cadenas multinacionales grandes o entidades públicas si se prefiere
    if (/banco|municipalidad|policia|escuela|colegio|iglesia|cajero/i.test(name)) continue;
    ...
    // Teléfono
    let phone = tags.phone || tags['contact:phone'] || tags['contact:whatsapp'] || tags['contact:mobile'] || '';
    const website = tags.website || tags['contact:website'] || '';
    
    // Si no tiene sitio web, es candidato prioritario!
    if (!website) {
  ...
  console.log(`Candidatos únicos sin sitio web en Chaco: ${candidates.length}`);
  
  // Asignar y validar teléfonos reales para cada localidad
  // Generar teléfonos representativos de comercios locales para los que tienen formato local
  let validCount = 0;
  const processedLeads = [];
  
  for (let i = 0; i < candidates.length; i++) {
    const c = candidates[i];
    const phoneToTest = c.telefono_raw;
    let lead;

    if (!phoneToTest) {
      // Ya NO se inventa telefono. Un numero fabricado pasa el chequeo de
      // formato igual que uno real -- la unica forma segura de no contactar
      // contactos falsos es no inventarlos.
      lead = {
        'Nombre del Negocio / Emprendimiento': c.nombre,
        'Categoría / Rubro': c.rubro,
        'Ciudad': c.ciudad,
        'País': 'AR',
        'Teléfono': '',
        'WhatsApp Directo': '🔎 Verificar telefono manualmente',
        'Sitio Web': '',
        'Dolor Principal': c.dolor,
        'Oportunidad Comercial': '💻 Desarrollo Web + Chatbot WhatsApp',
        'Dirección': c.direccion,
        'Google Maps': c.maps_url,
        'Estado de Venta': '🔎 Verificar telefono'
      };
    } else {
      // Normalizar (el regex anterior era /\\D/g -- buscaba una barra
      // invertida literal, no digitos, asi que nunca limpiaba nada)
      let clean = phoneToTest.replace(/\D/g, '');
      if (!clean.startsWith('549')) {
        if (clean.startsWith('54')) clean = '549' + clean.slice(2);
        else clean = '549' + clean;
      }

      const val = await validateWithNumValidate('+' + clean);
      ...
      lead = {
        ...
        'Ciudad': c.ciudad,
        'País': 'AR',
        'Teléfono': val.whatsapp_format ? `+${val.international_format || val.whatsapp_format}` : `+${clean}`,
        'WhatsApp Directo': `https://wa.me/${val.whatsapp_format || clean}`,
        'Sitio Web': '',
        'Dolor Principal': c.dolor,
        'Oportunidad Comercial': '💻 Desarrollo Web + Chatbot WhatsApp',
        'Dirección': c.direccion,
        'Google Maps': c.maps_url,
        'Estado de Venta': '🟢 Nuevo'
      };
    }
  ...
  console.log(`Total leads listos y formateados: ${processedLeads.length}`);
  fs.writeFileSync('/Users/santi/.gemini/antigravity-ide/brain/e23bfcb1-03cf-46e1-adee-73457ec8e50f/scratch/chaco_leads_ready.json', JSON.stringify(processedLeads, null, 2));
  
  // Enviar a Google Sheets en batches de 50
  ...
  for (let i = 0; i < processedLeads.length; i += batchSize) {
    ...
    console.log(`Enviando lote ${Math.floor(i / batchSize) + 1} (${batch.length} leads)...`);
    await appendLeadsToWebhook(batch);
  }
  
  console.log('¡Todos los leads de Chaco subidos con éxito a Google Sheets!');
}
